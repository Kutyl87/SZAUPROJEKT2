...
if reg_type==1
    ...
    for k=7:kk
        ...
        y_mod(k) = w20 + w2*tanh(w10 + w1*q(k,:)');
        b5 =  (w20 + w2*tanh(w10 + w1*q1(k,:)') - y_mod(k))/delta;
        b6 =  (w20 + w2*tanh(w10 + w1*q2(k,:)') - y_mod(k))/delta;
        b = [0,0,0,0,b5,b6];
        a1 =  - (w20 + w2*tanh(w10 + w1*q3(k,:)') - y_mod(k))/delta;
        a2 =  - (w20 + w2*tanh(w10 + w1*q4(k,:)') - y_mod(k))/delta;
        a = [a1,a2];
        d(k) = y_ob(k) -y_mod(k);
        Y_swobodne(1:N) = 0 ;
        for i=1:N
            if i>=3
                q_pred = [u(k+min(-1,-teta+i)), u(k+min(-1,-teta+i-1)), ...
                    Y_swobodne(i-1), Y_swobodne(i-2)];
            elseif i==2
                q_pred = [u(k+min(-1,-teta+i)), u(k+min(-1,-teta+i-1)), ...
                    Y_swobodne(i-1), y_ob(k)];
            else
                q_pred = [u(k+min(-1,-teta+i)), u(k+min(-1,-teta+i-1)), ... 
                    y_ob(k-1+i), y_ob(k-2+i)];
            end
...
for k=7:kk
    ...
    y_mod(k) = w20 + w2*tanh(w10 + w1*q(k,:)');
    d(k) = y_ob(k) -y_mod(k);
    U_predyktowane = ones(1, N) * u_predyktowane(1);
...
function [J] = dekorator(U_predyktowane)
    ...
    global w20
    global d
    J = NO_optimizer(yzad(global_k),y_ob,u, U_predyktowane, teta,N, Nu, lambda, global_k, w1,w2,w10,w20,d);
end