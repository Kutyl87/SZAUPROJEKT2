...
for k=7:kk
    ...
    for i=1:N
        if i>=teta+1
            q_pred = [U_swobodne(i-teta) U_swobodne(i-teta-1) Y_swobodne(i-1) Y_swobodne(i-2)];
        elseif i==teta
            q_pred = [U_swobodne(k-teta+i) u(k-teta+i-1) Y_swobodne(i-1) Y_swobodne(i-2)];
        elseif i>=3 && i< teta
            q_pred = [u(k-teta+i) u(k-teta+i-1) Y_swobodne(i-1) Y_swobodne(i-2)];
        elseif i==2
            q_pred = [u(k-teta+i) u(k-teta+i-1) Y_swobodne(i-1) y_ob(k)];
        else
            q_pred = [u(k-teta+i) u(k-teta+i-1) y_ob(k-1+i) y_ob(k-2+i)];
        end
